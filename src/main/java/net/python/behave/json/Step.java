    ...
    public String getName() {
        ...
        if (getStatus() == Util.Status.FAILED) {
            ...
            if (getStatus() == Util.Status.SKIPPED) {
                errorMessage[0] = "Mode: Skipped causes Failure<br/><span class=\"skipped\">This step was skipped</span>";
            }
            if (getStatus() == Util.Status.UNDEFINED) {
                errorMessage[0] = "Mode: Not Implemented causes Failure<br/><span class=\"undefined\">This step is not yet implemented</span>";
            }
            String joined = StringUtils.join(errorMessage);
            content = Util.result(getStatus()) + "<span class=\"step-keyword\">" + keyword + " </span><span class=\"step-name\">" + name + "</span><span class=\"step-duration\">" + Util.formatDuration(result.getDuration()) + "</span><div class=\"step-error-message\"><pre>" + formatError(joined) + "</pre></div>" + Util.closeDiv() + getImageTags();
